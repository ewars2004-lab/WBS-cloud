...
J〜O を書く・更新するときは [5w1h-customer-view.md](../wbs-update-shared/5w1h-customer-view.md) に従う。

- **What（M）** = お客様体験（「お客様が〜できる／〜が直る」）
- **Why（N）** = お客様への影響
- **How（O）** = お客様視点の検証・改善

B列（工程）ごとの文型テンプレは共有ガイドの「工程別テンプレ」を使う。会話に具体があればテンプレに足す。  
MD系・業務物流系はガイドの該当セクションを参照。
...
```json
{
  ...
  "updates": {
    ...
    "J": "2026/05/25以降。stg確認〜Android追加開発判明まで継続。",
    "K": "#pj_120303",
    "L": "Genさん、Ryoさん、Nagi。お客様はiOS先行リリース後の利用者。",
    "M": "お客様がiOS先行リリース後も安心して使えるか、Android追加開発の見通しが追える状態になる",
    "N": "OS別の状態が混ざると、お客様への提供タイミングが不透明になるため",
    "O": "審査・リリース連絡を追い、お客様体験への影響をWBSに反映する"
  },
...
- 大量更新時は **60 writes/分** の Sheets 制限に注意。1行ずつなら **1.2秒以上の間隔**、50行ごとに **65秒待機**
- 失敗行はレート制限解除後に再試行し、サマリーに含める
